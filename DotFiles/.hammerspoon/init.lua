...
for key, app in pairs(keyApps) do
	if key == string.upper(key) then
		hs.hotkey.bind(altCommandShift, key, function()
			hs.application.launchOrFocus(app)
		end)
	else
		hs.hotkey.bind(altCommand, key, function()
			hs.application.launchOrFocus(app)
		end)
	end
end
...
hs.hotkey.bind(altCommand, "z", function()
	undo:undo()
end)
...
local leftHalf = hs.hotkey.bind(altShift, "i", function()
	locationSet(0, 0, halfMaxWidth, hs.grid.GRIDHEIGHT)
end)
local rightHalf = hs.hotkey.bind(altShift, "o", function()
	locationSet(halfMaxWidth, 0, halfMaxWidth, hs.grid.GRIDHEIGHT)
end)

local fullScreen = hs.hotkey.bind(altShift, "f", function()
...
local upMove = hs.hotkey.bind(altShift, "k", function()
	straightlyMove(0, -1)
end)
local downMove = hs.hotkey.bind(altShift, "j", function()
	straightlyMove(0, 1)
end)
local leftMove = hs.hotkey.bind(altShift, "h", function()
	straightlyMove(-1, 0)
end)
local rightMove = hs.hotkey.bind(altShift, "l", function()
	straightlyMove(1, 0)
end)

local upExtend = hs.hotkey.bind(ctrlAlt, "k", function()
	straightlyExtendingWindow(0, -1)
end)
local downExtend = hs.hotkey.bind(ctrlAlt, "j", function()
	straightlyExtendingWindow(0, 1)
end)
local leftExtend = hs.hotkey.bind(ctrlAlt, "h", function()
	straightlyExtendingWindow(-1, 0)
end)
local rightExtend = hs.hotkey.bind(ctrlAlt, "l", function()
	straightlyExtendingWindow(1, 0)
end)
