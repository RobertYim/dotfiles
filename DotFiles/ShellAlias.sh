alias df='df -h'
alias systemLog='sudo less +F /var/log/system.log'
#alias xflux='~/Downloads/software/xflux -l 23.2 -g 113.2 -k 4600'
alias cp='cp -iv'
alias mv='mv -vi'
alias mkdir="mkdir -p"
alias ln='ln -v'
alias ...='../..'
alias roll='~/Documents/tools/roll.py'
# alias goagent='python ~/Downloads/goagent/local/proxy.py'
alias editHosts='sudo vim /etc/hosts'
alias you-get-proxy='proxychains4 python3 ~/Documents/development/repos/you-get/you-get'
alias you-get='python3 ~/Documents/development/repos/you-get/you-get'
#alias drbox='~/Documents/tools/Dropbox-Uploader/dropbox_uploader.sh'
#alias suspend.lock='~/Dropbox/dotfiles/suspend_script.sh'
#alias D='cd /media/niko/PersonalData/'
#alias E='cd /media/niko/Music'
#alias F='cd /media/niko/Movie'
#alias G='cd /media/niko/Storage'
alias e="$EDITOR"
alias v="$VISUAL"
alias t='~/Documents/tools/tTrans.py'
alias ip.search='~/Documents/tools/ip_search.py'
alias ping='ping -c 10'
alias gen.psd='~/Documents/tools/generate_password.py'
#alias remove_old_kernel='~/Dropbox/dotfiles/remove_old_kernel.sh'
alias h='history'
alias editAlias='vim /Users/niko/.oh-my-zsh/custom/bash_aliases.zsh'
alias editPac='vim ~/.ShadowsocksX/gfwlist.js'
alias tt='tmux attach'
alias l='ls -alh'
alias luTorrent="~/Documents/development/repos/LuTorrent/lutorrent.py"
alias getMyIP="curl -k --socks5-hostname localhost:1080 ipinfo.io"
alias rentCalculatior="python ~/Documents/tools/rent_calculator.py"
alias fetchMail="~/Documents/tools/mailinator.py"
alias bilibili="/Users/niko/Documents/development/repos/biligrab-danmaku2ass/bilidan.py"
alias WoW="cat ~/Dropbox/PDS/WOWO.txt"
alias baiduPan="~/Documents/development/repos/iScript/pan.baidu.com.py"
alias rsync="rsync -avP"
alias du="du -sh"

# Mac only
alias openFTPServer="sudo launchctl load -w /System/Library/LaunchDaemons/ftp.plist"
alias closeFTPServer="sudo launchctl unload -w /System/Library/LaunchDaemons/ftp.plist"
#alias openWinDisk='open /Volumes/win7/Users/niko/Downloads'
alias reloadSound='sudo kextunload /System/Library/Extensions/AppleHDA.kext && sudo kextload /System/Library/Extensions/AppleHDA.kext'
# alias flushDNS="sudo killall -HUP mDNSResponder" ## 10.9
alias flushDNS="sudo discoveryutil udnsflushcaches" ## 10.10
alias brewUpdateCleanup="brew update && brew upgrade brew-cask && brew cleanup && brew cask cleanup"
alias mountEFI="mkdir /Volumes/EFI && sudo mount -t msdos /dev/disk0s1 /Volumes/EFI"
alias wifi='networksetup -setairportnetwork en0 ccr629@fast 013257489@h'
alias vim='vimr'
